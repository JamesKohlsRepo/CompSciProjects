...
print()
print("Welcome to James Kohls's disease simualtor, Scenario 1, NO REGULATION ")
print(" -Dots may become infected if within 6ft of an infected sprite-")
print(" -When dots become sick, they will return home to self isolate-")
print()
print("1) How many people will populate the simulation?")
numberOfpeople = input("    (10-120 yields best results): ")                #Prompts the user for Number of dots
if numberOfpeople == "":                                            #check if nothing has been input
    numberOfpeople = 100                                            #If so it defaults to default values
...
patientOne = dot.Person((5,250), randc(), (255,165,0), infectivness = infectivity)     #Patient Zero is generated
infected_sprites.add(patientOne)                                                        #adds to the lisd


for i in range(0, numberOfpeople):
    if i <= numberOfpeople/2:
        i = dot.Person((i * int(500/(numberOfpeople/2)), 490), randc(), infectivness = infectivity)
    else:
        i = dot.Person((i * int(500/(numberOfpeople/2))-500, 5), randc(), infectivness = infectivity)
...
while running:
    ...
    for aPerson in all_sprites:
        ...
        aPerson.collide(infected_sprites)
        aPerson.collide(sick_sprites)
