...
print()
print("Welcome to James Kohls's disease simualtor, Scenario 3, SHELTER IN PLACE ")
print(" -On day one, 90% of dots will practice social distancing-")
print(" -On day one, 75% of dots will shelter in place-")
...
for i in range(0, numberOfpeople):
    ...
    if randPic <= 9:
        ...
        sd = 0
    randPic2 = random.randrange(0, 100)  # Determines the Deadliness of the disease
    if randPic2 <= 75:
         sp = 1
    else:
         sp = 0
    if i <= numberOfpeople/2:
        i = dot.Person((i * int(500/(numberOfpeople/2)), 490), randc(), infectivness = infectivity, SocialDistancing = sd, covid2 = 1, covid3 = sp)
    else:
        i = dot.Person((i * int(500/(numberOfpeople/2))-500, 5), randc(), infectivness = infectivity, SocialDistancing = sd, covid2 = 1, covid3 = sp)
